<div class="footer">
            <div class="container">
                <b class="copyright">&copy; 2014 H1CMS - H1CMS.COM </b> All rights reserved.
            </div>
        </div>
<script src="<?php echo base_url('/themes/default/admin/scripts/jquery-1.9.1.min.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/scripts/jquery-ui-1.10.1.custom.min.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/bootstrap/js/bootstrap.min.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/scripts/flot/jquery.flot.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/scripts/flot/jquery.flot.resize.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/scripts/datatables/jquery.dataTables.js'); ?>" type="text/javascript"></script>
<script src="<?php echo base_url('/themes/default/admin/scripts/common.js'); ?>" type="text/javascript"></script>

</body>
</html>
